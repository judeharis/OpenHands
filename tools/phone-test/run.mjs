...
if (mode === "plan") {
  ...
  note("ux", subId ? `plan mode created a second sandbox (sub-conversation ${subId}); ready ${((Date.now()-tPlan)/1000).toFixed(0)}s after the Plan tap` : "planning sub-conversation never became ready");
}
...
let sentFollowUp = false; let sawRunning = false; let approvals = 0, lastApprovalAt = Date.now(), scrolls = 0, missingPanel = 0, built = false, finalText = "";
...
while (Date.now() < deadline) {
  ...
  if (st === "waiting_for_confirmation") {
    ...
    note("confirm", `#${approvals + 1} after ${wait}s: ${title}`);
...
fs.writeFileSync(`${OUT}/log.json`, JSON.stringify({ cid, mode, task, approvals, scrolls, missingPanel, totalSec: (Date.now()-t0)/1000, log, finalText }, null, 1));
await browser.close();
